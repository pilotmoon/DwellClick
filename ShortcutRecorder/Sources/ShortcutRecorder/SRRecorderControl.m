       ...
       allowsEmptyModifierFlags:(BOOL)newAllowsEmptyModifierFlags
{
    ...
    if (newAllowedModifierFlags == _allowedModifierFlags && newRequiredModifierFlags == _requiredModifierFlags && newAllowsEmptyModifierFlags == _allowsEmptyModifierFlags) {
        return;
    }
...
- (void)updateLabelConstraints
{
    ...
    _labelWidthConstraint.constant = ceil(MAX(labelWidth,
                                              [labelAttributes[SRMinimalDrawableWidthAttributeName] doubleValue])
        + 2.0);
}
...
- (void)drawLabel:(NSRect)aDirtyRect
{
    ...
    NSString *label = self.drawingLabel;
    // NM modification
    NSMutableDictionary *labelAttributes = [self.drawingLabelAttributes mutableCopy];
    if (self.smallFont) {
        labelAttributes[NSFontAttributeName] = [NSFont systemFontOfSize:[NSFont systemFontSizeForControlSize:NSControlSizeSmall]];
    }
    ...
    labelFrame = [self backingAlignedRect:labelFrame
                                  options:NSAlignRectFlipped | NSAlignMinXOutward | NSAlignMinYOutward | NSAlignMaxXInward |
        NSAlignMaxYInward];
...
#pragma clang diagnostic pop
    ...
    os_activity_initiate("-[SRRecorderControl areModifierFlagsAllowed:forKeyCode:]", OS_ACTIVITY_FLAG_IF_NONE_PRESENT, ^{
        if ((aModifierFlags == 0 && !self.allowsEmptyModifierFlags) || ((aModifierFlags & self.allowedModifierFlags) != aModifierFlags)) {
...
- (void)propagateValue:(id)aValue forBinding:(NSString *)aBinding
{
    ...
    if (!_isCompatibilityModeEnabled && ([boundObject isKindOfClass:NSUserDefaults.class] || [boundObject isKindOfClass:NSUserDefaultsController.class]) &&
    ...
    os_activity_initiate("-[SRRecorderControl performKeyEquivalent:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        if (self.isRecording) {
            if (anEvent.keyCode == USHRT_MAX) {
                ...
                result = NO;
            } else if (self.allowsEscapeToCancelRecording && anEvent.keyCode == SRKeyCodeEscape && (anEvent.modifierFlags & SRCocoaModifierFlagsMask) == 0) {
                os_log_debug(OS_LOG_DEFAULT, "Cancel via Esc");
                [self endRecording];
                result = YES;
            } else if (self.allowsDeleteToClearShortcutAndEndRecording && (anEvent.keyCode == SRKeyCodeDelete || anEvent.keyCode == SRKeyCodeForwardDelete) && (anEvent.modifierFlags & SRCocoaModifierFlagsMask) == 0) {

                       ...
                       context:(void *)aContext
{
    ...
    os_activity_initiate("-[SRShortcutAction observeValueForKeyPath:ofObject:change:context:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        if ([newShortcut isKindOfClass:NSDictionary.class])
            ...
            newShortcut = [NSKeyedUnarchiver unarchivedObjectOfClass:[SRShortcut class] fromData:(NSData *)newShortcut error:nil];
                                ...
                                modifierFlags:(NSEventModifierFlags)aModifierFlags
{
    ...
    case NSEventTypeFlagsChanged: {
        ...
        if (keyCode == kVK_Command || keyCode == kVK_RightCommand)
            ...
            os_log_debug(OS_LOG_DEFAULT, "#Error Unexpected key code %hu for the FlagsChanged event", keyCode);
        break;
    }
    default:
...
- (OSStatus)handleEvent:(EventRef)anEvent
{
    ...
    os_activity_initiate("-[SRGlobalShortcutMonitor handleEvent:]", OS_ACTIVITY_FLAG_DETACHED, ^{
        ...
        @synchronized(self->_actions) {
            ...
            if (!shortcut) {
                os_log_debug(OS_LOG_DEFAULT, "Unregistered hot key with id %u and signature %u", hotKeyID.id, hotKeyID.signature);
                return;
            }
            ...
            case kEventHotKeyReleased:
                ...
                break;
            default:
                os_log_debug(OS_LOG_DEFAULT, "#Error Unexpected key event of type %u", GetEventKind(anEvent));
                return;
            }
...
- (void)_installEventHandlerIfNeeded
{
    ...
    os_log_debug(OS_LOG_DEFAULT, "Installing Carbon hot key event handler");
...
- (void)_removeEventHandlerIfNeeded
{
    ...
    os_log_debug(OS_LOG_DEFAULT, "Removing Carbon hot key event handler");
...
- (void)_registerHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    os_log_debug(OS_LOG_DEFAULT, "Registering Carbon hot key");
    ...
    os_log_debug(OS_LOG_DEFAULT, "Registered Carbon hot key %u", hotKeyID.id);
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    os_log_debug(OS_LOG_DEFAULT, "Removing Carbon hot key %u", hotKeyID);
    ...
    if (error != noErr) {
        ...
        os_log_debug(OS_LOG_DEFAULT, "Unregistered Carbon hot key %u", hotKeyID);
    }
...
{
    static const CGEventMask Mask = (CGEventMaskBit(kCGEventKeyDown) | CGEventMaskBit(kCGEventKeyUp) | CGEventMaskBit(kCGEventFlagsChanged));
...
{
    __block CGEventRef result = anEvent;
